...
int32_t app_debug_rtt(const char *fmt,...)
{
    ...
    if (n > (int)size) 
    {
    	User_Uart_AsyncTransmit(USER_UART_CHANNEL_USART1, m_debug_buffer, size + time_stamp_size);
        //SEGGER_RTT_Write(0, m_debug_buffer, size + time_stamp_size);
    } 
    else if (n > 0) 
    {
    	User_Uart_AsyncTransmit(USER_UART_CHANNEL_USART1, m_debug_buffer, n + time_stamp_size);
